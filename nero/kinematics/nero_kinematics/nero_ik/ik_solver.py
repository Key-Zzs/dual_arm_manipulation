...
def _theta0_scan_step(n_psi: int) -> float:
    """Return full-range arm-angle scan step from a requested grid count."""
    return 2.0 * math.pi / max(31, int(n_psi))
...
def ik_arm_angle(
    ...
    `n_psi` controls the full-range arm-angle grid count.
    """
    if p is None:
        p = NeroParams.default()

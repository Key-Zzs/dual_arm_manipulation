'''
Nero dual-arm robot local execution server.
Provides the migrated robot-control methods used by the WBCD execution layer.
'''
...
from wbcd_task1.execution.kinematics.nero_solver import Pinocchio_Solver
...
def start_server(gripper_enabled: bool = True, tcp_offset_enabled: bool = False, limit_z: float = 0.07):
    """Create the local Nero execution server object.

    The function keeps the old name for operator muscle memory, but it no
    longer opens a network endpoint.
    """
    return NeroDualArmServer(gripper_enabled, tcp_offset_enabled, limit_z)
...
if __name__ == '__main__':
    ...
    start_server(gripper_enabled=args.no_gripper, tcp_offset_enabled=args.tcp_offset_enabled, limit_z=args.limit_z)
